 ...
 *
 * now add args inside brackets, lol.
 * **********************************
 * 
    Requires input validation then onto structs, yay.
 * 
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
char fnargs[MAXTOKEN];    /* last token string */
int faflag=0;
...
{
    int c, i=0, getch(void);   /* Note the function prototype getch*/
    void ungetch(int);
    char *p = token;
...
void dirdcl(void)
{
    ...
    while((type=gettoken())==PARENS || type == BRACKETS || type==NAME) 
    {
        if(type==PARENS) 
        {
            ...
            } else strcat(out, " function returning");
        }
        else if(type==NAME) 
        {
            strcat(datatype," ");
